...
from interfaces import Event, Scene, CharacterInScene, CharacterInEvent, CharacterInNovel
...
class GlobalInformationPlanner:
    ...
    async def merge_characters_across_scenes_in_event(
        ...
        for character in characters_in_event:
            for scene_idx, identifier_in_scene in character.active_scenes.items():
                if identifier_in_scene not in [c.identifier_in_scene for c in scenes[scene_idx].characters]:
                    raise ValueError(
                        f"Character {identifier_in_scene} not found in scene {scene_idx} of event {event_idx}")
                else:
        ...
        for scene_idx, flag in enumerate(flags):
            for identifier_in_scene, included in flag.items():
                if not included:
                    raise ValueError(
                        f"Character {identifier_in_scene} in scene {scene_idx} of event {event_idx} not included in the merged characters")
    ...
    def merge_characters_to_existing_characters_in_novel(
        ...
        chain = self.chat_model | parser
        response: MergeCharactersAcrossScenesInEventResponse = chain.invoke(messages)
...
# TODO：如果是长篇小说，事件太多，很容易出现：出场的角色不知道在哪个事件中登场，很容易漏
